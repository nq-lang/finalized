...
# ─────────────────────────────────────────────────────────────────────────────
# SABR + ANALYTICAL HELPER FUNCTIONS
# ─────────────────────────────────────────────────────────────────────────────

def _sabr_vol(F: float, K: float, T: float,
              alpha: float, beta: float, rho: float, nu: float) -> float:
    """SABR implied vol (Hagan 2002). Handles ATM/OTM, includes 0DTE tau floor."""
    if T <= 0 or F <= 0 or K <= 0 or alpha <= 0:
        return max(alpha, 0.001)
    import math
    eps = 1e-8
    FK_b = (F * K) ** ((1 - beta) / 2)
    log_FK = math.log(F / K) if abs(F - K) > eps else 0.0
    z = (nu / alpha) * FK_b * log_FK if abs(log_FK) > eps else 0.0
    if abs(z) < eps:
        chi = 1.0
    else:
        denom = math.sqrt(1 - 2*rho*z + z**2) + z - rho
        if denom <= 0:
            chi = 1.0
        else:
            chi = math.log(denom / (1 - rho))
        chi = z / chi if abs(chi) > eps else 1.0
    atm_vol = alpha / FK_b
    expansion = (
        1
    ...
    if abs(log_FK) > eps:
        wing = (1
                ...
                + (1-beta)**4/1920 * log_FK**4)
        atm_vol = atm_vol / wing
    return float(max(atm_vol * chi * expansion, 0.001))


def _fit_sabr_lm(strikes: list, ivs: list, F: float, T: float,
                 beta: float = 0.5) -> dict:
    """Calibrate SABR (alpha, rho, nu) via scipy least_squares (Levenberg-Marquardt)."""
    try:
        from scipy.optimize import least_squares
        import numpy as np
        if len(strikes) < 4:
            return {"alpha": 0.20, "beta": beta, "rho": -0.30,
                    "nu": 0.40, "ok": False, "rmse": 9.99}
        atm_iv = float(np.interp(F, strikes, ivs)) if min(strikes) < F < max(strikes) else float(np.mean(ivs))
        x0 = [max(atm_iv, 0.05), -0.30, 0.40]
        bounds = ([0.005, -0.995, 0.005], [3.0, 0.995, 8.0])
        ...
        res = least_squares(_resid, x0, bounds=bounds, method="trf",
                            ftol=1e-8, xtol=1e-8, gtol=1e-8, max_nfev=600)
        a, r, n = res.x
        rmse = float(np.sqrt(np.mean(np.array(res.fun)**2)))
        return {"alpha": float(a), "beta": beta, "rho": float(r),
                "nu": float(n), "ok": bool(res.success), "rmse": rmse,
                "cost": float(res.cost)}
    except Exception as e:
        return {"alpha": 0.20, "beta": beta, "rho": -0.30,
                "nu": 0.40, "ok": False, "rmse": 9.99}
...
def _bs_speed(S: float, K: float, T: float, r: float, q: float, sigma: float) -> float:
    """Analytical Speed = dGamma/dS = -Gamma/S * (d1/(sigma*sqrt(T)) + 1)"""
    import math
    d1, _ = _d1d2(S, K, T, r, q, sigma)
    if np.isnan(d1) or T <= 0 or sigma <= 0 or S <= 0:
        return 0.0
    g = bs_gamma(S, K, T, r, q, sigma)
    return float(-g / S * (d1 / (sigma * math.sqrt(T)) + 1))


def _bs_color(S: float, K: float, T: float, r: float, q: float, sigma: float) -> float:
    """Analytical Color = dGamma/dT (closed-form, no bumping)."""
    import math
    d1, d2 = _d1d2(S, K, T, r, q, sigma)
    if np.isnan(d1) or T <= 0 or sigma <= 0:
        return 0.0
    g = bs_gamma(S, K, T, r, q, sigma)
    return float(-g * (2*(r-q)*T + 1 + d1*(2*(r-q)*T - d2*sigma*math.sqrt(T))) / (2*T))


def _micro_price(bid: float, ask: float,
                 bid_sz: float, ask_sz: float) -> float:
    """Bayesian micro-price: P = (Bid*AskVol + Ask*BidVol)/(BidVol+AskVol)."""
    total = bid_sz + ask_sz
    if total <= 0:
        return (bid + ask) / 2.0
    return (bid * ask_sz + ask * bid_sz) / total
...
# ─────────────────────────────────────────────────────────────────────────────
# INSTITUTIONAL DEALER HEDGING SURFACES — v2.0
# Net Notional Exposure: Greek × OI × Spot scaling → true dealer impact
# ─────────────────────────────────────────────────────────────────────────────

def render_dealer_hedging_surfaces(ticker: str, spot: float,
                                   raw_df: pd.DataFrame, T: dict):
    """
    Institutional-grade dealer hedging surfaces:

    Net Exposure Definition (institutional standard):
      GEX = Gamma × OI × 100 × Spot² × 0.01 / 1e9   [$B dealer impact per 1% move]
      DEX = Delta × OI × 100 × Spot / 1e6            [$M notional delta]
      VEX = Vanna × OI × 100 × Spot / 1e6            [$M vanna-vol feedback]
      SPD = Speed × OI × 100 × Spot³ × 0.01 / 1e12  [liquidity vacuum metric]
      CLR = Color × OI × 100 × Spot² × 0.01 / 1e9   [gamma time-decay metric]

    Models:
      1. GEX + DEX 3-D surface — SABR calibrated per expiry
      2. VEX (Vanna) surface   — Spot-vol feedback / melt-up detector
      3. Speed surface         — dGamma/dS analytical, liquidity vacuum
      4. Color surface         — dGamma/dT analytical, afternoon drift timer

    All IVs computed via SABR (Hagan 2002) with Levenberg-Marquardt calibration.
    Micro-price used for IV input (not mid-price) to eliminate toxic flow bias.
    """
    import math, datetime as _dt2

    _BG   = T.get("bg",   "#03070D")
    _BG2  = T.get("bg2",  "#050E1A")
    _TXT  = T.get("t1",   "#E8EDF5")
    _TXT3 = T.get("t3",   "#4A5568")
    _GRN  = T.get("green","#00E5A0")
    _RED  = T.get("red",  "#FF3860")
    _AMB  = T.get("amber","#F5A623")
    _LINE = T.get("line", "#0D1F38")
    _LINE2= T.get("line2","#122540")
    ...
    _fut_abbr, _fut_name = _ASSET_FUT.get(ticker, (ticker, ticker))
    _q  = _DIV_SCHEDULE.get(ticker, 0.01)

    st.markdown(f"""
    <div style="background:{_BG};border:1px solid {_LINE2};border-radius:4px;
                padding:12px 20px;margin-bottom:12px;border-left:4px solid {_GRN};">
      <div style="display:flex;align-items:baseline;gap:16px;">
        <span style="font-family:'Barlow Condensed',sans-serif;font-size:13px;font-weight:700;
                     letter-spacing:2px;text-transform:uppercase;color:{_GRN};">
          DEALER HEDGING SURFACES</span>
        <span style="font-family:JetBrains Mono,monospace;font-size:9px;color:{_TXT3};">
          {ticker} · {_fut_name} · SPOT ${spot:,.2f} · SABR + Analytical Closed-Form</span>
      </div>
      <div style="font-family:JetBrains Mono,monospace;font-size:7.5px;color:{_TXT3};
                  margin-top:4px;letter-spacing:0.5px;">
        Net Notional Exposure = Greek × OI × Spot-scaling  ·  Micro-price IV inputs  ·
        Speed/Color: Analytical partial derivatives (no bumping)  ·
        SABR: Levenberg-Marquardt calibration per expiry
      </div>
    </div>
    """, unsafe_allow_html=True)

    if raw_df is None or raw_df.empty:
        st.warning("No options data. Load from GEX Dashboard first.")
        return

    # ── Build micro-price-adjusted chain ─────────────────────────────────────
    _cdf = raw_df.copy()
    _bid  = _cdf.get("bid",  pd.Series(0.0, index=_cdf.index)).fillna(0)
    _ask  = _cdf.get("ask",  pd.Series(0.0, index=_cdf.index)).fillna(0)
    _bsz  = _cdf.get("bid_size", pd.Series(1.0, index=_cdf.index)).fillna(1)
    _asz  = _cdf.get("ask_size", pd.Series(1.0, index=_cdf.index)).fillna(1)
    _cdf["_mp"] = [_micro_price(b, a, bs, as_)
                   for b, a, bs, as_ in zip(_bid, _ask, _bsz, _asz)]

    today = _dt2.date.today()

    # ── Per-expiry SABR calibration ───────────────────────────────────────────
    _exps = sorted(_cdf["expiry"].unique()) if "expiry" in _cdf.columns else []
    _sabr_fit = {}    # exp → sabr params dict
    _fwd_map  = {}    # exp → forward price

    for _exp in _exps[:8]:
        try:
            _days = (_dt2.datetime.strptime(_exp, "%Y-%m-%d").date() - today).days
            if _days < 0:
                continue
            _tau = max(_days / 365.0, 30 / 86400)  # 0DTE floor: 30 seconds
            _rt  = _sofr_rate(_days)

            # Forward price via put-call parity
            _exp_df = _cdf[_cdf["expiry"] == _exp]
            _calls  = _exp_df[_exp_df.get("flag", _exp_df.get("option_type", pd.Series())).isin(["C","call"]) if "flag" in _exp_df.columns or "option_type" in _exp_df.columns else pd.Series([True]*len(_exp_df), index=_exp_df.index)]
            _puts   = _exp_df[~_exp_df.index.isin(_calls.index)]
            _F      = _forward_price_pcp(spot, _tau, _rt, _q,
                                         _calls.rename(columns={"option_type":"flag"}) if "option_type" in _calls.columns else _calls,
                                         _puts.rename(columns={"option_type":"flag"})  if "option_type" in _puts.columns  else _puts)
            _fwd_map[_exp] = _F

            # Build IV smile from micro-prices
            _stks_iv, _ivs_fit = [], []
            for _, row in _exp_df.iterrows():
                K   = float(row.get("strike", 0) or 0)
                oi  = float(row.get("open_interest", 0) or 0)
                mp  = float(row.get("_mp", 0) or 0)
                raw_iv = float(row.get("iv", 0) or 0)
                fl  = str(row.get("flag", row.get("option_type", "C")))

                if K <= 0 or oi < 50:
                    continue
                if abs(K - spot) / spot > 0.10:
                    continue  # wing filter

                # IV: prefer micro-price implied, fallback to raw
                iv = np.nan
                if mp > 0.05 and _tau > 0:
                    iv_mp = implied_vol(mp, spot, K, _tau, _rt, _q, fl)
                    if iv_mp and not np.isnan(iv_mp) and 0.03 < iv_mp < 3.0:
                        iv = iv_mp
                if np.isnan(iv) and raw_iv > 0.03:
                    iv = raw_iv

                if not np.isnan(iv) and 0.03 < iv < 3.0:
                    _stks_iv.append(K)
                    _ivs_fit.append(iv)

            if len(_stks_iv) >= 5:
                _sp = _fit_sabr_lm(_stks_iv, _ivs_fit, _F, _tau, beta=0.5)
                _sp.update({"F": _F, "tau": _tau, "days": _days})
                _sabr_fit[_exp] = _sp
        except Exception:
            continue
    ...
    _N_STK  = 44
    _pct_rng = np.linspace(-0.08, 0.08, _N_STK)
    _K_grid  = [spot * (1 + p) for p in _pct_rng]
    _K_labs  = [f"${k:,.0f} ({p:+.1%})" for k, p in zip(_K_grid, _pct_rng)]

    _use_exps = sorted(_sabr_fit.keys()) if _sabr_fit else _exps[:6]
    _n_exp    = len(_use_exps)

    if _n_exp == 0:
        st.warning("No calibrated expirations available.")
        return

    # ── Net exposure matrices ─────────────────────────────────────────────────
    _Z_gex = np.zeros((_n_exp, _N_STK))
    _Z_dex = np.zeros((_n_exp, _N_STK))
    _Z_van = np.zeros((_n_exp, _N_STK))
    _Z_spd = np.zeros((_n_exp, _N_STK))
    _Z_clr = np.zeros((_n_exp, _N_STK))

    for _ei, _exp in enumerate(_use_exps):
        try:
            _sp   = _sabr_fit.get(_exp, {})
            _tau  = _sp.get("tau", 30/86400)
            _days = _sp.get("days", 0)
            _rt   = _sofr_rate(_days)
            _F    = _sp.get("F", spot)
            _ok   = _sp.get("ok", False)

            # OI maps (calls positive, puts negative in dealer framework)
            ...
            for _, row in _exp_df.iterrows():
                ...
                if K > 0 and oi > 0:
                    if fl in ("C", "call"):
                        _oi_c[K] = _oi_c.get(K, 0) + oi
                    else:
                        _oi_p[K] = _oi_p.get(K, 0) + oi

            for _ki, K in enumerate(_K_grid):
                try:
                    # SABR IV for this strike
                    if _ok and _sp.get("alpha"):
                        iv = _sabr_vol(_F, K, _tau,
                                       _sp["alpha"], _sp["beta"],
                                       _sp["rho"], _sp["nu"])
                        iv = float(np.clip(iv, 0.03, 3.0))
                    else:
                        # Fallback: nearest raw IV
                        if not _exp_df.empty and "iv" in _exp_df.columns:
                            _nr = _exp_df.iloc[(_exp_df["strike"] - K).abs().argsort()].iloc[0]
                            iv  = float(max(_nr.get("iv", 0.20) or 0.20, 0.03))
                        else:
                            iv = 0.20

                    # Nearest OI from chain
                    _Kn  = min(list(_oi_c.keys() or [K]) + list(_oi_p.keys() or [K]),
                               key=lambda k: abs(k - K))
                    _oic = _oi_c.get(_Kn, 0)
                    _oip = _oi_p.get(_Kn, 0)

                    # Greeks (calls long gamma, puts short gamma for dealer)
                    g   = bs_gamma(spot, K, _tau, _rt, _q, iv)
                    dc  = bs_delta(spot, K, _tau, _rt, _q, iv, "C")
                    dp  = bs_delta(spot, K, _tau, _rt, _q, iv, "P")
                    van = bs_vanna(spot, K, _tau, _rt, _q, iv)
                    spd = _bs_speed(spot, K, _tau, _rt, _q, iv)
                    clr = _bs_color(spot, K, _tau, _rt, _q, iv)

                    # Net notional exposure (institutional scaling)
                    # GEX: $B per 1% move in spot
                    _Z_gex[_ei, _ki] = (g * _oic - g * _oip) * 100 * spot**2 * 0.01 / 1e9
                    # DEX: $M notional delta
                    _Z_dex[_ei, _ki] = (dc * _oic + dp * _oip) * 100 * spot / 1e6
                    # VEX: $M vanna-vol feedback
                    _Z_van[_ei, _ki] = (van * (_oic + _oip)) * 100 * spot / 1e6
                    # Speed: $B rate-of-gamma change (liquidity vacuum)
                    _Z_spd[_ei, _ki] = (spd * (_oic - _oip)) * 100 * spot**3 * 0.01 / 1e12
                    # Color: $B gamma time-decay (drift timer)
                    _Z_clr[_ei, _ki] = (clr * (_oic - _oip)) * 100 * spot**2 * 0.01 / 1e9
                except Exception:
                    pass
        except Exception:
            continue
    ...
    _y_labs = [
        f"{e} · {_sabr_fit[e]['days'] if e in _sabr_fit else '?'}DTE"
        for e in _use_exps
    ]

    # ── Colorscales ───────────────────────────────────────────────────────────
    _cs_gex = [[0,"#8B0000"],[0.2,"#CC3300"],[0.38,"#FF6600"],
               [0.5,"#111111"],[0.62,"#007744"],[0.8,"#00CC77"],[1,"#00E5A0"]]
    _cs_dex = [[0,"#0A0020"],[0.3,"#2233BB"],[0.5,"#333333"],
               [0.7,"#BB2266"],[1,"#FF3399"]]
    _cs_van = [[0,"#050510"],[0.25,"#1A1A99"],[0.45,"#6622CC"],
               [0.5,"#222222"],[0.55,"#CC4400"],[0.75,"#FF8800"],[1,"#FFDD00"]]
    _cs_spd = [[0,"#001020"],[0.3,"#004488"],[0.5,"#222222"],
               [0.7,"#884400"],[1,"#FFAA00"]]
    _cs_clr = [[0,"#001500"],[0.35,"#005522"],[0.5,"#111111"],
               [0.65,"#883300"],[1,"#FF4400"]]
    ...
    def _render_surface(Z, title, cs, units, note=""):
        _fig = go.Figure()
        _fig.add_trace(go.Surface(
            z=Z, x=_K_labs, y=_y_labs,
            colorscale=cs,
            cmid=0,
            colorbar=dict(
            ...
            lighting=dict(ambient=0.65, diffuse=0.75, specular=0.25,
        ...
        # Zero-plane (dealer neutral level)
        _fig.add_trace(go.Surface(
            z=np.zeros_like(Z), x=_K_labs, y=_y_labs,
        ...
        _fig.update_layout(
            ...
            height=500,
            margin=dict(t=52, r=10, b=10, l=10),
            title=dict(
                text=f"<b>{title}</b>  ·  {ticker} / {_fut_name}  ·  {note}",
            ...
            scene=dict(
                bgcolor=_BG,
                xaxis=dict(
                    title="Strike", tickfont=dict(size=7, color=_TXT3),
                    gridcolor=_LINE, showbackground=True, backgroundcolor=_BG,
                    tickangle=-30,
                ),
                yaxis=dict(
                    title="Expiry", tickfont=dict(size=7, color=_TXT3),
                    gridcolor=_LINE, showbackground=True, backgroundcolor=_BG,
                ),
                zaxis=dict(
                    title=units, tickfont=dict(size=7, color=_TXT3),
                    gridcolor=_LINE, showbackground=True, backgroundcolor=_BG,
                ),
            ...
            font=dict(family="JetBrains Mono,monospace", color=_TXT, size=9),
            hoverlabel=dict(bgcolor=_BG2, bordercolor=_LINE2,
                            font=dict(family="JetBrains Mono", size=9, color=_TXT)),
        )
        return _fig

    def _section_header(num, title, subtitle, signal_html=""):
        st.markdown(f"""
        <div style="padding:10px 0 6px 0;border-top:1px solid {_LINE2};margin-top:14px;">
          <div style="display:flex;align-items:baseline;gap:14px;">
            <span style="font-family:'Barlow Condensed',sans-serif;font-size:11px;
                         font-weight:700;color:{_GRN};letter-spacing:2px;">
              {num}</span>
            <span style="font-family:'Barlow Condensed',sans-serif;font-size:18px;
                         font-weight:700;color:{_TXT};letter-spacing:1px;text-transform:uppercase;">
              {title}</span>
            <span style="font-family:JetBrains Mono,monospace;font-size:8px;color:{_TXT3};">
              {subtitle}</span>
          </div>
          {signal_html}
        </div>
    ...
    # ── SABR Calibration Status ───────────────────────────────────────────────
    if _sabr_fit:
        _cal_data = []
        for _e, _sp in list(_sabr_fit.items())[:8]:
            _regime = ("Steep downside skew" if _sp["rho"] < -0.45 else
                       "Normal skew"          if _sp["rho"] < -0.20 else
                       "Flat / mildly bullish")
            _cal_data.append({
                "Expiry": _e, "DTE": _sp.get("days","?"),
                "α (vol level)": f"{_sp['alpha']:.4f}",
                "β (backbone)":  f"{_sp['beta']:.2f}",
                "ρ (skew)":      f"{_sp['rho']:+.3f}",
                "ν (vol-of-vol)":f"{_sp['nu']:.3f}",
                "RMSE": f"{_sp.get('rmse',9.99):.5f}",
                "Fit": "✓ LM" if _sp.get("ok") else "✗ fallback",
                "Regime": _regime,
                "Fwd $": f"{_sp.get('F', spot):,.2f}",
            })
        st.markdown(
            ...
            f'SABR Calibration — {len(_cal_data)} expirations · '
            f'Levenberg-Marquardt · Micro-price IV inputs</div>',
            unsafe_allow_html=True)
        st.dataframe(pd.DataFrame(_cal_data), use_container_width=True,
                     hide_index=True, height=min(35*len(_cal_data)+40, 250))

    # ── SURFACE 1: GEX + DEX ─────────────────────────────────────────────────
    _section_header("①", "GEX & DEX SURFACES",
                    "Gamma + Delta Net Notional Exposure · SABR calibrated",
                    f'<div style="font-family:JetBrains Mono,monospace;font-size:8px;'
                    f'color:{_TXT3};margin-top:4px;">'
                    f'GEX = Γ×OI×100×S²×0.01/1e9 [$B/1%] &nbsp;|&nbsp; '
                    f'DEX = Δ×OI×100×S/1e6 [$M notional]</div>')

    _gc1, _gc2 = st.columns(2)
    with _gc1:
        _fg = _render_surface(_Z_gex, "GEX — Dealer Gamma Exposure", _cs_gex, "$B/1% move",
                              f"Net = Call GEX − Put GEX  ·  {_n_exp} expirations")
        st.plotly_chart(_fg, use_container_width=True, config=_surf_cfg)
    with _gc2:
        _fd = _render_surface(_Z_dex, "DEX — Delta Notional Exposure", _cs_dex, "$M notional Δ",
                              f"Net = (ΔC×OI_C + ΔP×OI_P)×100×S/1e6")
        st.plotly_chart(_fd, use_container_width=True, config=_surf_cfg)

    # ── SURFACE 2: VANNA — Spot-Vol Loop ─────────────────────────────────────
    _net_vanna_tot = float(np.nansum(_Z_van))
    _rv_vix = _compute_realized_vol("VIX")
    _vix_hv = _rv_vix.get("hv10", 0.15)
    _meltup = _net_vanna_tot > 0 and _vix_hv > 0.22
    _meltdown = _net_vanna_tot < 0 and _vix_hv > 0.22
    _van_signal = ""
    if _meltup:
        _van_signal = (f'<div style="background:rgba(0,229,160,0.08);border:1px solid {_GRN};'
                       f'border-radius:3px;padding:7px 12px;margin-top:6px;'
                       f'font-family:JetBrains Mono,monospace;font-size:8.5px;color:{_GRN};">'
                       f'⚡ VANNA MELT-UP SIGNAL — Net Vanna positive ({_net_vanna_tot:+.2f}) '
                       f'+ VIX HV-10 elevated ({_vix_hv:.1%}). '
                       f'Dealers forced long underlying as vol drops. Watch for squeeze.</div>')
    elif _meltdown:
        _van_signal = (f'<div style="background:rgba(255,56,96,0.08);border:1px solid {_RED};'
                       f'border-radius:3px;padding:7px 12px;margin-top:6px;'
                       f'font-family:JetBrains Mono,monospace;font-size:8.5px;color:{_RED};">'
                       f'⚠️ VANNA UNWIND SIGNAL — Net Vanna negative ({_net_vanna_tot:+.2f}) '
                       f'+ VIX elevated. Dealers sell underlying as vol rises. Potential acceleration.</div>')

    _section_header("②", "VANNA SURFACE",
                    "dΔ/dσ — Spot-Vol Feedback · Melt-up/Unwind Engine",
                    f'<div style="font-family:JetBrains Mono,monospace;font-size:8px;'
                    f'color:{_TXT3};margin-top:4px;">'
                    f'VEX = Vanna×OI×100×S/1e6 [$M] &nbsp;|&nbsp; '
                    f'Net Vanna: <span style="color:{"#00E5A0" if _net_vanna_tot > 0 else "#FF3860"};">'
                    f'{_net_vanna_tot:+.3f} $M</span>'
                    f'</div>' + _van_signal)

    _fv = _render_surface(_Z_van, "VANNA — dΔ/dσ Exposure", _cs_van, "Vanna $M",
                          "High Vanna + VIX drop → Dealer forced long → Melt-up")
    st.plotly_chart(_fv, use_container_width=True, config=_surf_cfg)

    # ── SURFACE 3: SPEED — Liquidity Vacuum ──────────────────────────────────
    _spd_peak_idx = tuple(np.unravel_index(np.abs(_Z_spd).argmax(), _Z_spd.shape))
    _spd_peak_stk = _K_labs[_spd_peak_idx[1]] if _spd_peak_idx[1] < len(_K_labs) else "?"
    _spd_peak_exp = _y_labs[_spd_peak_idx[0]] if _spd_peak_idx[0] < len(_y_labs) else "?"

    _section_header("③", "SPEED SURFACE",
                    "dΓ/dS — Gamma-of-Gamma · Liquidity Vacuum Detector",
                    f'<div style="font-family:JetBrains Mono,monospace;font-size:8px;'
                    f'color:{_TXT3};margin-top:4px;">'
                    f'Speed = −Γ/S × (d₁/(σ√τ) + 1)  ·  Closed-form, no finite-diff bumping  ·  '
                    f'Peak: {_spd_peak_stk} / {_spd_peak_exp}</div>')

    _fs = _render_surface(_Z_spd, "SPEED — dΓ/dS (Liquidity Vacuum)", _cs_spd, "Speed $B",
                          "Spike zones = exponential dealer hedging → slippage / gap-risk")
    st.plotly_chart(_fs, use_container_width=True, config=_surf_cfg)

    # ── SURFACE 4: COLOR — Afternoon Drift Timer ──────────────────────────────
    _clr_peak_idx = tuple(np.unravel_index(np.abs(_Z_clr).argmax(), _Z_clr.shape))
    _clr_peak_stk = _K_labs[_clr_peak_idx[1]] if _clr_peak_idx[1] < len(_K_labs) else "?"

    _section_header("④", "COLOR SURFACE",
                    "dΓ/dτ — Gamma Time Decay · Afternoon Drift Timer",
                    f'<div style="font-family:JetBrains Mono,monospace;font-size:8px;'
                    f'color:{_TXT3};margin-top:4px;">'
                    f'Color = ∂Γ/∂τ closed-form  ·  '
                    f'High Color → dealer hedges expiring → mean-revert in final 90 min  ·  '
                    f'Peak unwind zone: {_clr_peak_stk}</div>')

    _fc = _render_surface(_Z_clr, "COLOR — dΓ/dτ (Drift Timer)", _cs_clr, "Color $B",
                          "Gamma time-decay map → identifies mechanical bid exhaustion zones")
    st.plotly_chart(_fc, use_container_width=True, config=_surf_cfg)

    # ── Net Exposure Summary Table ────────────────────────────────────────────
    st.markdown(f"""
    <div style="font-family:JetBrains Mono,monospace;font-size:8px;color:{_TXT3};
                letter-spacing:1.5px;text-transform:uppercase;
                padding:10px 0 6px 0;border-top:1px solid {_LINE2};margin-top:14px;">
      NET EXPOSURE SUMMARY  ·  Aggregated across all calibrated expirations
    </div>
    """, unsafe_allow_html=True)

    _net_rows = [
        {"Metric": "GEX Net",   "Value": f"{np.nansum(_Z_gex):+.4f} $B",
         "Regime": "Long Γ → stabilising" if np.nansum(_Z_gex) > 0 else "Short Γ → destabilising"},
        {"Metric": "DEX Net",   "Value": f"{np.nansum(_Z_dex):+.2f} $M",
         "Regime": "Net long Δ" if np.nansum(_Z_dex) > 0 else "Net short Δ"},
        {"Metric": "VEX Net",   "Value": f"{np.nansum(_Z_van):+.3f} $M",
         "Regime": "Melt-up risk" if _meltup else "Unwind risk" if _meltdown else "Neutral"},
        {"Metric": "Speed Peak","Value": _spd_peak_stk,
         "Regime": "Liquidity vacuum / gap zone"},
        {"Metric": "Color Peak","Value": _clr_peak_stk,
         "Regime": "Mechanical bid exhaustion zone"},
    ]
    st.dataframe(pd.DataFrame(_net_rows), use_container_width=True,
...
def dashboard():
    ...
    if st.session_state.radar_mode == "GEX":
        ...
        # ══════════════════════════════════════════════════════════════════════
        # 5-GREEK STACKED BAR CHART PANELS
        # Each Greek displayed as its own horizontal bar chart
        # Listed vertically, each with: strike range selector + Greek selector
        # Hover: Strike price, underlying asset conversion, exposure value
        # Positive/Negative walls clearly color-coded
        # ══════════════════════════════════════════════════════════════════════

        # ── Asset → futures mapping for hover display ─────────────────────────
        _ASSET_FUT_LABELS = {
            "SPX": ("ES Futures", "ES"),
            "SPY": ("ES Futures", "ES"),
            "NDX": ("NQ Futures", "NQ"),
        ...
        _fut_name, _fut_abbr = _ASSET_FUT_LABELS.get(asset_toggle, (asset_toggle, asset_toggle))
        _ratio = equiv_mult if equiv_mult and equiv_mult > 0 else 1.0
        _spot_fut_val = spot_price * _ratio

        # ── Strike range selector (persists across greeks) ────────────────────
        if "strike_range" not in st.session_state:
            st.session_state.strike_range = 30
        _range_opts = [20, 25, 30, 40, 50, 60, 80, 100]

        _rng_cols = st.columns(len(_range_opts) + 1)
        for _ri, _rv in enumerate(_range_opts):
            with _rng_cols[_ri]:
                def _mk_rng(rv):
                    def _cb(): st.session_state.strike_range = rv
                    return _cb
                st.button(str(_rv), key=f"srange_{_rv}",
                    type="primary" if st.session_state.strike_range == _rv else "secondary",
                    on_click=_mk_rng(_rv), use_container_width=True)
        with _rng_cols[-1]:
            st.markdown(
                f'<div style="font-family:JetBrains Mono,monospace;font-size:7.5px;'
                f'color:{T["t3"]};padding:9px 0;letter-spacing:1.5px;text-transform:uppercase;">'
                f'STRIKES ±{st.session_state.strike_range//2} FROM SPOT</div>',
                unsafe_allow_html=True)

        _n_stk_sel = st.session_state.strike_range
        _greek_tab  = st.session_state.greek_tab   # currently selected greek

        # ── Build filtered strike universe ────────────────────────────────────
        _greek_col_map = {
            "GEX": ("gex_net", "#00E5A0", "#FF3860", "GEX $B",    "Gamma Exposure — Dealer Net"),
            "DEX": ("dex_net", "#4488FF", "#FF66AA", "DEX Δ$M",   "Delta Exposure — Notional"),
            "VEX": ("vex_net", "#AA44FF", "#FF9944", "VEX $M",    "Vanna Exposure — Sticky Δ"),
            "CEX": ("cex_net", "#FFB800", "#6644FF", "CEX",       "Charm Exposure — Sticky Δ"),
            "TEX": ("tex_net", "#FF4466", "#4488FF", "TEX $K/day","Theta Exposure — Decay"),
        }
        _greek_order = ["GEX", "DEX", "VEX", "CEX", "TEX"]

        if df.empty:
            st.info("No options data loaded.")
        else:
            _pdf_base = df.copy()
            _pdf_base["_fut_k"] = _pdf_base["strike"] * _ratio
            # Underlying-equivalent (options strike in underlying price)
            _pdf_base["_und_k"] = _pdf_base["strike"]

            _all_fut_stk = sorted(_pdf_base["_fut_k"].unique())
            if not _all_fut_stk:
                st.info("No strikes available.")
            else:
                # Find nearest future strike to spot
                _si = min(range(len(_all_fut_stk)), key=lambda i: abs(_all_fut_stk[i] - _spot_fut_val))
                _half = _n_stk_sel // 2
                _keep_stk = set(_all_fut_stk[max(0, _si - _half):_si + _half])
                _pdf = _pdf_base[_pdf_base["_fut_k"].isin(_keep_stk)].sort_values("_fut_k").reset_index(drop=True)

                # Spot label for shape overlays
                _spot_lbl = f"${min(_pdf['_fut_k'].tolist(), key=lambda s: abs(s - _spot_fut_val)):,.0f}" if not _pdf.empty else ""

                # ── Render each Greek as its own full-width panel ─────────────
                for _gk in _greek_order:
                    _gcn, _cp, _cn, _gl, _gdesc = _greek_col_map[_gk]
                    _is_active = (_gk == _greek_tab)

                    # Per-Greek header with its own strike range + active indicator
                    _hdr_col1, _hdr_col2, _hdr_col3 = st.columns([2, 6, 2])
                    with _hdr_col1:
                        st.markdown(
                            f'<div style="background:{"rgba(0,229,160,0.08)" if _is_active else T["bg1"]}; '
                            f'border:1px solid {"#00E5A0" if _is_active else T["line2"]}; '
                            f'border-radius:4px;padding:8px 12px;margin-bottom:4px;">'
                            f'<div style="font-family:Barlow Condensed,sans-serif;font-size:20px;'
                            f'font-weight:700;color:{"#00E5A0" if _is_active else T["t1"]};">{_gk}</div>'
                            f'<div style="font-family:JetBrains Mono,monospace;font-size:7px;'
                            f'color:{T["t3"]};letter-spacing:1px;margin-top:2px;">{_gdesc}</div></div>',
                            unsafe_allow_html=True)
                    with _hdr_col2:
                        st.markdown(
                            f'<div style="font-family:JetBrains Mono,monospace;font-size:8px;'
                            f'color:{T["t3"]};padding:8px 0;letter-spacing:1px;">'
                            f'{asset_toggle}  ·  {_fut_abbr} equiv  ·  {_n_stk_sel} strikes  ·  '
                            f'Spot ${spot_price:,.2f}  ·  {_fut_abbr} ${_spot_fut_val:,.1f}</div>',
                            unsafe_allow_html=True)
                    with _hdr_col3:
                        def _mk_gk_cb(g):
                            def _cb(): st.session_state.greek_tab = g
                            return _cb
                        st.button(f"{'★ ' if _is_active else ''}{_gk} ACTIVE",
                                  key=f"gk_active_{_gk}",
                                  type="primary" if _is_active else "secondary",
                                  on_click=_mk_gk_cb(_gk),
                                  use_container_width=True)

                    if _gcn not in _pdf.columns:
                        st.info(f"No {_gk} data. Engine may need more expirations.")
                        continue

                    _vals = _pdf[_gcn].fillna(0).tolist()
                    _fut_stks  = _pdf["_fut_k"].tolist()
                    _und_stks  = _pdf["_und_k"].tolist()
                    _ylabs = [f"${s:,.0f}" for s in _fut_stks]

                    # Bar colors: positive wall / negative wall
                    _bar_cols = [_cp if v >= 0 else _cn for v in _vals]

                    # Identify significant walls (top 20% abs value)
                    _abs_thresh = float(pd.Series(_vals).abs().quantile(0.80)) if _vals else 0
                    _wall_sizes = [1.5 if abs(v) >= _abs_thresh and _abs_thresh > 0 else 0.3 for v in _vals]
                    ...
                    _fig.add_trace(go.Bar(
                        ...
                        marker=dict(
                            color=_bar_cols,
                            opacity=[0.92 if abs(v) >= _abs_thresh and _abs_thresh > 0 else 0.65 for v in _vals],
                            line=dict(
                                color=_bar_cols,
                                width=_wall_sizes,
                            ),
                        ...
                        customdata=list(zip(
                            _fut_stks,
                            [f"{asset_toggle} ${u:,.2f} / {_fut_abbr} ${f:,.1f}"
                             for u, f in zip(_und_stks, _fut_stks)],
                            _vals,
                            [_cp if v >= 0 else _cn for v in _vals],
                            ["CALL WALL" if v >= _abs_thresh and _abs_thresh > 0
                             else "PUT WALL" if v <= -_abs_thresh and _abs_thresh > 0
                             else "" for v in _vals],
                        )),
                        hovertemplate=(
                            f"<b>%{{customdata[4]}} {_gk}</b><br>"
                            f"Strike ({_fut_abbr}): $%{{customdata[0]:,.1f}}<br>"
                            f"Asset: %{{customdata[1]}}<br>"
                            f"{_gl}: %{{customdata[2]:+.5f}}<br>"
                            "<extra></extra>"
                        ),
                        showlegend=False,
                    ))
                    ...
                    _fig.add_vline(x=0, line_color=T["line2"], line_width=1.0, line_dash="solid")

                    # Spot horizontal line
                    if _spot_lbl in _ylabs:
                        _fig.add_shape(
                        ...
                        _fig.add_annotation(
                            xref="paper", yref="y",
                            x=1.0, y=_spot_lbl,
                            text=f" SPOT {spot_price:.2f} / {_fut_abbr} {_spot_fut_val:,.1f}",
                            showarrow=False,
                            font=dict(size=8, color=T["amber"], family="JetBrains Mono"),
                            xanchor="left", yanchor="middle",
                        )

                    # Major wall annotations (top positive + top negative)
                    _pdf_temp = _pdf.copy()
                    _pdf_temp["_v"] = _vals
                    _pdf_temp["_yl"] = _ylabs
                    _top_pos = _pdf_temp[_pdf_temp["_v"] > 0].nlargest(3, "_v")
                    _top_neg = _pdf_temp[_pdf_temp["_v"] < 0].nsmallest(3, "_v")

                    for _, _wr in _top_pos.iterrows():
                        _fig.add_annotation(
                            xref="x", yref="y",
                            x=float(_wr["_v"]), y=_wr["_yl"],
                            text=f" ▲ CALL WALL ${_wr['_und_k']:,.0f}",
                            showarrow=False,
                            font=dict(size=7, color=_cp, family="JetBrains Mono"),
                            xanchor="left", yanchor="middle",
                        )
                    for _, _wr in _top_neg.iterrows():
                        _fig.add_annotation(
                            xref="x", yref="y",
                            x=float(_wr["_v"]), y=_wr["_yl"],
                            text=f"▼ PUT WALL ${_wr['_und_k']:,.0f} ",
                            showarrow=False,
                            font=dict(size=7, color=_cn, family="JetBrains Mono"),
                            xanchor="right", yanchor="middle",
                        )

                    _panel_h = max(300, min(_n_stk_sel * 18 + 60, 650))
                    _fig.update_layout(
                        template="none",
                        paper_bgcolor=T["bg"],
                        plot_bgcolor=T["bg1"] if _is_active else T["bg"],
                        height=_panel_h,
                        margin=dict(t=28, r=180, b=28, l=88),
                        xaxis=dict(
                            title=dict(text=_gl, font=dict(size=8, color=T["t3"])),
                            gridcolor=T["line"],
                            tickfont=dict(size=8, family="JetBrains Mono", color=T["t3"]),
                            zeroline=True,
                            zerolinecolor=T["line2"],
                        ...
                        yaxis=dict(
                            gridcolor="rgba(0,0,0,0)",
                            tickfont=dict(size=8, family="JetBrains Mono", color=T["t2"]),
                        ),

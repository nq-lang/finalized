...
def render_greeks_3d_page():
    ...
    for greek_key, greek_label, colorscale_3d in _SURFACES:
        ...
        fig_3d = go.Figure(go.Surface(
            ...
            hovertemplate=(f"Strike: %{{x:.2f}}<br>Expiry: %{{y}}<br>"
                           f"{greek_label}: %{{z:.4f}}<extra></extra>"),
        ...
        fig_3d.update_layout(
            ...
            scene=dict(
                xaxis=dict(title=dict(text="Strike", font=dict(color=TEXT3_3, size=9)),
                           **_axis_style_3),
...
_INTRA_SYM_MAP = {
    ...
    "SPY": {"futures": "SPY",   "label": "SPY",        "opt": "SPY",  "ratio_fn": "spy"},
    "GLD": {"futures": "GC=F",  "label": "Gold (GC)", "opt": "GLD",  "ratio_fn": "gld"},
    "VIX": {"futures": "^VIX",  "label": "VIX Spot",  "opt": "SPX",  "ratio_fn": "spx"},
}
...
def dashboard():
    ...
    if st.session_state.radar_mode == "GEX":
        ...
        _sym_fut = {
            ...
            "SPY": ("SPY","SPY"),         "GLD": ("GC=F","Gold/GC"),
            "VIX": ("^VIX","VIX Spot"),
        }
        fut_tk, fut_lbl = _sym_fut.get(asset_toggle, (asset_toggle, asset_toggle))

        # ── MODEL 1: Intraday GEX Intensity (candlestick + level overlay) ─────
        st.markdown(
            f'<div style="font-family:JetBrains Mono,monospace;font-size:9px;'
            f'color:{T["t3"]};letter-spacing:2px;text-transform:uppercase;'
        ...
        st.markdown(
            ...
            f'Positive | Neutral | Negative GEX  ·  Strike × DTE</div>',
            unsafe_allow_html=True)
        ...
        if _gs[0] is not None:
            ...
            fig_gex3d = go.Figure(go.Surface(
                ...
                hovertemplate="Strike: %{x:.2f}<br>Expiry: %{y}<br>GEX: %{z:.5f}B<extra></extra>",
            ...
            fig_gex3d.update_layout(
                paper_bgcolor=T["bg"], height=500,
                margin=dict(l=0, r=0, b=0, t=30),
                scene=dict(
                    xaxis=dict(title=dict(text="Strike", font=dict(color=T["t3"],size=9)),**_ax3g),
                    yaxis=dict(title=dict(text="DTE",    font=dict(color=T["t3"],size=9)),**_ax3g),
                    zaxis=dict(title=dict(text="GEX",    font=dict(color=T["t3"],size=9)),**_ax3g),
                    camera=dict(eye=dict(x=1.6,y=-1.7,z=0.9), up=dict(x=0,y=0,z=1)),
                    aspectmode="manual", aspectratio=dict(x=2.5,y=0.8,z=0.7),
                ),
                showlegend=False,
                font=dict(family="JetBrains Mono, monospace", color=T["t2"], size=9),
                title=dict(text=f"{asset_toggle}  ·  Gamma Exposure Surface",
                           font=dict(size=11, color=T["t1"], family="JetBrains Mono"),
                           x=0.01),
            )
        ...
        def _build_trace_heatmap(ticker: str, sym_ratio: float,
            ...
            try:
                import json as _j
                raw_local = pd.read_json(_j.loads(raw_df_json), orient="split")
            except Exception:
                return None, None, None
        ...
        try:
            ...
            _Z_hm, _strikes_hm, _times_hm = _build_trace_heatmap(
                asset_toggle, float(equiv_mult), _jmod.dumps(_raw_json))
        except Exception:

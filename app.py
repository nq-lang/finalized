...
_PAGES = {
    ...
    "macro_shi":    "⚡  MACRO TYPE SHI",
    "vix_regime":   "🔮  VIX REGIME / HMM",
}
...
def dashboard():
    ...
    if st.session_state.radar_mode == "GEX":
        ...
        if df_intra.empty:
            ...
            # Gamma intensity background bands
            for _, row_g in _top5_pos.iterrows():
                lvl = float(row_g["strike"]) * _sym_ratio
                intensity = min(abs(float(row_g["gex_net"])) / _max_g, 1.0)
                if y_lo_i*0.97 <= lvl <= y_hi_i*1.03:
                    fig_ig.add_shape(type="rect", x0=0, x1=1, xref="paper",
                                     y0=lvl - spacing*0.5, y1=lvl + spacing*0.5, yref="y",
                                     fillcolor=f"rgba(0,229,160,{0.04 + 0.12*intensity:.3f})",
                                     line=dict(width=0), layer="below")
            for _, row_g in _top5_neg.iterrows():
                lvl = float(row_g["strike"]) * _sym_ratio
                intensity = min(abs(float(row_g["gex_net"])) / _max_g, 1.0)
                if y_lo_i*0.97 <= lvl <= y_hi_i*1.03:
                    fig_ig.add_shape(type="rect", x0=0, x1=1, xref="paper",
                                     y0=lvl - spacing*0.5, y1=lvl + spacing*0.5, yref="y",
                                     fillcolor=f"rgba(255,56,96,{0.04 + 0.12*intensity:.3f})",
                                     line=dict(width=0), layer="below")

            # Candlestick
            fig_ig.add_trace(go.Candlestick(
                x=x_i, open=df_intra["open"], high=df_intra["high"],
                low=df_intra["low"], close=df_intra["close"],
                increasing_line_color=T["green"], decreasing_line_color=T["red"],
                increasing_fillcolor=T["green"],  decreasing_fillcolor=T["red"],
                line_width=0.9, showlegend=False,
            ...
            # Top-5 positive GEX level lines (intensity-weighted width)
            for i_, (_, rg) in enumerate(_top5_pos.iterrows()):
                lvl = float(rg["strike"]) * _sym_ratio
                intensity = min(abs(float(rg["gex_net"])) / _max_g, 1.0)
                lw = 0.6 + 1.6 * intensity
                if y_lo_i*0.97 <= lvl <= y_hi_i*1.03:
                    fig_ig.add_hline(y=lvl, row=1, col=1,
                                     line_color=T["green"], line_width=lw, line_dash="solid")
                    fig_ig.add_annotation(
                        x=1.0, y=lvl, xref="paper", yref="y",
                        text=f" +{i_+1} ${lvl:,.0f} {rg['gex_net']:+.3f}B ",
                        showarrow=False, xanchor="left",
                        font=dict(size=7.5, color=T["bg"], family="JetBrains Mono"),
                        bgcolor=T["green"], borderpad=2, opacity=0.85+0.12*intensity,
                    )

            # Top-5 negative GEX level lines
            for i_, (_, rg) in enumerate(_top5_neg.iterrows()):
                lvl = float(rg["strike"]) * _sym_ratio
                intensity = min(abs(float(rg["gex_net"])) / _max_g, 1.0)
                lw = 0.6 + 1.6 * intensity
                if y_lo_i*0.97 <= lvl <= y_hi_i*1.03:
                    fig_ig.add_hline(y=lvl, row=1, col=1,
                                     line_color=T["red"], line_width=lw, line_dash="solid")
                    fig_ig.add_annotation(
                        x=1.0, y=lvl, xref="paper", yref="y",
                        text=f" -{i_+1} ${lvl:,.0f} {rg['gex_net']:+.3f}B ",
                        showarrow=False, xanchor="left",
                        font=dict(size=7.5, color=T["bg"], family="JetBrains Mono"),
                        bgcolor=T["red"], borderpad=2, opacity=0.85+0.12*intensity,
                    )

            # Key structural levels
            for lvl_v, lvl_c, lvl_dash, lvl_lw, lvl_nm in [
                (gamma_flip * _sym_ratio, T["amber"],  "dot",  1.4, "Zero Γ"),
                (call_wall  * _sym_ratio, T["green"],  "dash", 1.6, "Call Wall"),
                (put_wall   * _sym_ratio, T["red"],    "dash", 1.6, "Put Wall"),
                (spot_i,                  T["t1"],     "solid",1.8, "LAST"),
            ]:
                if y_lo_i*0.97 <= lvl_v <= y_hi_i*1.03:
                    fig_ig.add_hline(y=lvl_v, row=1, col=1,
                                     line_color=lvl_c, line_width=lvl_lw,
                                     line_dash=lvl_dash)
                    fig_ig.add_annotation(
                        x=1.0, y=lvl_v, xref="paper", yref="y",
                        text=f"  {lvl_nm} {lvl_v:,.1f} ",
                        showarrow=False, xanchor="left",
                        font=dict(size=8, color=T["bg"] if lvl_nm=="LAST" else lvl_c,
                                  family="JetBrains Mono"),
                        bgcolor=lvl_c if lvl_nm=="LAST" else T["bg1"],
                        bordercolor=lvl_c, borderwidth=1, borderpad=3, opacity=0.92,
                    )
            ...
            fig_ig.update_layout(
                ...
                font=dict(family="JetBrains Mono, monospace", color=T["t2"], size=10),
                showlegend=False, height=560, xaxis_rangeslider_visible=False,
                uirevision=f"intraday_{asset_toggle}",
                margin=dict(t=16, r=150, b=36, l=62), bargap=0.1,
                hoverlabel=dict(bgcolor=T["bg2"], bordercolor=T["line_bright"],
                                font=dict(family="JetBrains Mono", size=10, color=T["t1"])),
            )
            fig_ig.update_xaxes(
                type="category", showgrid=True, gridcolor=T["chart_line"], gridwidth=1,
                tickfont=dict(size=8, color=T["t3"]),
                showspikes=True, spikemode="across",
                spikethickness=1, spikecolor=T["t3"], spikedash="dot", nticks=12,
            )
            y_pad_i = (y_hi_i - y_lo_i) * 0.04
            fig_ig.update_yaxes(
                showgrid=True, gridcolor=T["chart_line"], gridwidth=1,
        ...
        st.markdown(
            ...
            f'margin-top:10px;">{asset_toggle}  ·  3-D GAMMA EXPOSURE SURFACE  ·  '
            f'Positive (above) | Negative (below) zero  ·  Strike × DTE  ·  '
            f'Hover shows {equiv_label} price</div>',
            unsafe_allow_html=True)

        _gs = _build_greek_matrix(raw_df, spot_price, "gamma")
        if _gs[0] is not None:
            _g_strikes, _g_expiries, _g_Z = _gs
            _g_arr = np.array(_g_Z, dtype=float)
            # Also compute converted (futures) strikes for hover
            _g_strikes_fut = [round(s * float(equiv_mult), 2) for s in _g_strikes]
            _exp_lbls_3d = [str(e)[-5:] if len(str(e))>10 else str(e) for e in _g_expiries]
            _ax3g = dict(backgroundcolor=T["bg1"], gridcolor=T["line2"], gridwidth=1,
                         showspikes=False, zeroline=False,
                         tickfont=dict(size=7, color=T["t3"], family="JetBrains Mono"))
            # Diverging colorscale: red neg → black zero → blue-white pos
            _gex_cs = [
                [0.00,"#FF2244"],[0.18,"#CC0033"],[0.35,"#660018"],
                [0.46,"#220008"],[0.50,T["bg"]],
                [0.54,"#001433"],[0.65,"#004488"],[0.82,"#2266CC"],[1.00,"#88CCFF"],
            ]
            # customdata: futures-converted strike for hover display
            _cd = np.array(_g_strikes_fut * len(_exp_lbls_3d)).reshape(
                len(_exp_lbls_3d), len(_g_strikes)).T

            fig_gex3d = go.Figure(go.Surface(
                x=_g_strikes, y=_exp_lbls_3d, z=_g_arr.T,
                colorscale=_gex_cs, showscale=True, opacity=0.93,
                customdata=_cd,
                colorbar=dict(
                ...
                lighting=dict(ambient=0.6, diffuse=0.9, specular=0.15, roughness=0.55),
            ))
            # Spot & GammaFlip lines
            for _lk, _lc, _ln in [
                (min(_g_strikes, key=lambda k: abs(k-spot_price)), T["t1"],   "SPOT"),
                (min(_g_strikes, key=lambda k: abs(k-gamma_flip)), T["amber"],"ZERO Γ"),
            ]:
                _idx = _g_strikes.index(_lk)
                _zl  = [float(_g_arr[_idx,i]) for i in range(len(_exp_lbls_3d))]
                fig_gex3d.add_trace(go.Scatter3d(
                    x=[_lk]*len(_exp_lbls_3d), y=_exp_lbls_3d, z=_zl,
                    mode="lines", line=dict(color=_lc, width=3),
                    hoverinfo="skip",
                ))
            fig_gex3d.update_layout(
                paper_bgcolor=T["bg"], height=520,
                margin=dict(l=0, r=0, b=0, t=30),
                scene=dict(
                    xaxis=dict(title=dict(text=f"{asset_toggle} Strike ($)",
                                          font=dict(color=T["t3"],size=9)),**_ax3g),
                    yaxis=dict(title=dict(text="DTE",
                                          font=dict(color=T["t3"],size=9)),**_ax3g),
                    zaxis=dict(title=dict(text="GEX ($B)",
                                          font=dict(color=T["t3"],size=9)),
                               zeroline=True, zerolinecolor=T["t1"], zerolinewidth=2,
                               **{k:v for k,v in _ax3g.items() if k not in ("zeroline",)}),
                    camera=dict(eye=dict(x=1.6,y=-1.7,z=0.9), up=dict(x=0,y=0,z=1)),
                    aspectmode="manual", aspectratio=dict(x=2.5,y=0.8,z=0.7),
                ),
                showlegend=False,
                font=dict(family="JetBrains Mono, monospace", color=T["t2"], size=9),
                title=dict(text=(
                    f"{asset_toggle}  ·  Gamma Exposure Surface  ·  "
                    f"<span style='font-size:9px;color:{T['t3']};'>"
                    f"Positive GEX above zero plane (stable) · Negative below (volatile) · "
                    f"Hover = {equiv_label} converted price</span>"),
                    font=dict(size=10, color=T["t1"], family="JetBrains Mono"),
                    x=0.01),
            )
            st.plotly_chart(fig_gex3d, use_container_width=True, config={
                "displayModeBar": True, "displaylogo": False, "scrollZoom": False,
                "modeBarButtonsToRemove": [
                    "sendDataToCloud","toImage","zoom3d","pan3d",
            ...
            st.info("GEX surface data unavailable.")

        # ── MODEL 3: SpotGamma-style TRACE GEX Heatmap ───────────────────────
        st.markdown(
            f'<div style="font-family:JetBrains Mono,monospace;font-size:9px;'
            f'color:{T["t3"]};letter-spacing:2px;text-transform:uppercase;'
            f'padding:12px 0 6px 0;border-top:1px solid {T["line2"]};'
            f'margin-top:10px;">{asset_toggle}  ·  GEX TRACE HEATMAP  ·  '
            f'Delta Pressure · Time × Strike · Price Overlay</div>',
            unsafe_allow_html=True)

        # Build the 2D GEX × Time heatmap matrix
        # Axes: X = intraday time bars (using df_intra), Y = strike prices
        # Color: GEX intensity per strike × time (diverging: blue=+, red=-)
        @st.cache_data(ttl=60, show_spinner=False)
        ...
        try:
            _raw_json = raw_df.to_json(orient="split")
            _Z_hm, _strikes_hm, _times_hm = _build_trace_heatmap(
                asset_toggle, float(equiv_mult), _raw_json)
        except Exception:
            _Z_hm, _strikes_hm, _times_hm = None, None, None

        if _Z_hm is None:
            st.info("TRACE heatmap data unavailable — try again during market hours.")
        else:
            # Normalise per-row for visual contrast
            _row_max = np.abs(_Z_hm).max(axis=1, keepdims=True)
            _row_max = np.where(_row_max < 1e-12, 1e-12, _row_max)
            _Z_norm  = (_Z_hm / _row_max).astype(np.float32)

            # SpotGamma diverging colorscale
            # Deep blue = strong +GEX (stability), deep red = strong -GEX (volatility)
            # White = neutral (zero crossing)
            _trace_cs = [
                [0.000, "#AA0000"], [0.080, "#CC1122"], [0.180, "#EE2244"],
                [0.300, "#FF5577"], [0.400, "#FFAACC"], [0.480, "#FFE0EE"],
                [0.500, "#FFFFFF"],
                [0.520, "#E0EEFF"], [0.600, "#AACCFF"], [0.700, "#5588EE"],
                [0.820, "#2244CC"], [0.920, "#1133AA"], [1.000, "#001188"],
            ]

            fig_trace = go.Figure()

            # Heatmap layer
            fig_trace.add_trace(go.Heatmap(
                z=_Z_norm,
                x=_times_hm,
                y=_strikes_hm,
                colorscale=_trace_cs,
                zmid=0, zmin=-1.0, zmax=1.0,
                zsmooth="best",
                colorbar=dict(
                    title=dict(text="Delta ($B Notional)",
                               font=dict(size=8, color=T["t3"],
                                         family="JetBrains Mono")),
                    tickfont=dict(size=7.5, color=T["t3"], family="JetBrains Mono"),
                    tickvals=[-1.0, -0.5, 0.0, 0.5, 1.0],
                    ticktext=["-2B", "-1B", "0", "+1B", "+2B"],
                    thickness=12, len=0.75,
                    x=1.01,
                ),
                hovertemplate=(
                    "Time: <b>%{x}</b><br>"
                    "Strike: <b>$%{y:,.0f}</b><br>"
                    "GEX Intensity: <b>%{z:.3f}</b><extra></extra>"
                ),
                xgap=0, ygap=0,
            ))

            # Contour lines overlay (topographic-style boundaries)
            # Topographic isoline overlay using manual scatter contours (safe approach)
            # Draw white boundary lines at ±0.4 and ±0.7 GEX intensity levels
            for _iso_val, _iso_opacity in [(-0.7, 0.25), (-0.4, 0.18),
                                            (0.4, 0.18),  (0.7, 0.25)]:
                _iso_col = f"rgba(255,255,255,{_iso_opacity})"
                # Find strike indices near this isoline per time step
                _iso_x, _iso_y = [], []
                for _tj, _t in enumerate(_times_hm):
                    for _si in range(len(_strikes_hm)-1):
                        v0, v1 = float(_Z_norm[_si, _tj]), float(_Z_norm[_si+1, _tj])
                        if (v0 - _iso_val) * (v1 - _iso_val) < 0:
                            # Linear interpolation
                            frac = (_iso_val - v0) / (v1 - v0 + 1e-12)
                            _iso_x.append(_t)
                            _iso_y.append(_strikes_hm[_si] + frac *
                                          (_strikes_hm[_si+1] - _strikes_hm[_si]))
                if _iso_x:
                    fig_trace.add_trace(go.Scatter(
                        x=_iso_x, y=_iso_y, mode="markers",
                        marker=dict(color=_iso_col, size=1.5, opacity=_iso_opacity),
                        hoverinfo="skip", showlegend=False,
                    ))

            # Live price trace overlay
            if not df_intra.empty:
                _px_times  = df_intra["ts"].dt.strftime("%H:%M").tolist()
                _px_closes = (df_intra["close"] * float(equiv_mult)).tolist()
                fig_trace.add_trace(go.Scatter(
                    x=_px_times, y=_px_closes,
                    mode="lines",
            ...
            # Key GEX levels as horizontal lines
            for _lv, _lc, _ln in [
                (gamma_flip * float(equiv_mult), T["amber"],  "Zero Γ"),
                (call_wall  * float(equiv_mult), "#00FF88",   "Call Wall"),
                (put_wall   * float(equiv_mult), "#FF4466",   "Put Wall"),
            ]:
                if min(_strikes_hm) * 0.97 <= _lv <= max(_strikes_hm) * 1.03:
                    fig_trace.add_hline(
                        y=_lv, line_dash="dot", line_color=_lc, line_width=1.4,
                        annotation_text=f" {_ln} ${_lv:,.0f}",
                        annotation_position="right",
                        annotation_font=dict(size=8, color=_lc,
                                             family="JetBrains Mono"),
                    )
            ...
            fig_trace.update_layout(
                ...
                font=dict(family="JetBrains Mono, monospace", color=T["t2"], size=9),
                height=560,
                margin=dict(t=28, r=90, b=50, l=72),
                uirevision=f"trace_{asset_toggle}",
                xaxis=dict(
                    title=dict(text="Time (ET)", font=dict(size=9, color=T["t3"])),
                    gridcolor=T["line2"], gridwidth=1,
                    tickfont=dict(size=8, color=T["t3"]),
                    nticks=14,
                ),
                yaxis=dict(
                    title=dict(text="Strike / Price ($)",
                               font=dict(size=9, color=T["t3"])),
                    gridcolor=T["line2"], gridwidth=1,
                    tickfont=dict(size=9, color=T["t3"]),
                    tickprefix="$",
                    range=[spot_price * 0.953, spot_price * 1.047],
                ),
                showlegend=False,
                title=dict(
                    text=(f"<b>{asset_toggle}  GEX TRACE</b>  ·  "
                          f"SpotGamma-style  ·  "
                          f"<span style='color:{T['t3']};font-size:9px;'>"
                          f"Blue=+GEX(stable)  Red=-GEX(volatile)  "
                          f"White=price trace</span>"),
                    font=dict(size=10, color=T["t1"], family="JetBrains Mono"),
                    x=0.01, xanchor="left",
                ),
                hoverlabel=dict(
            ...
            st.plotly_chart(fig_trace, use_container_width=True, config={
                "displayModeBar": True, "displaylogo": False, "scrollZoom": True,
                "modeBarButtonsToRemove": ["sendDataToCloud", "lasso2d", "select2d"],
            })

            # GEX by Strike bar (left panel, matches SpotGamma layout)
            st.markdown(
                f'<div style="font-family:JetBrains Mono,monospace;font-size:8px;'
                f'color:{T["t3"]};letter-spacing:2px;text-transform:uppercase;'
                f'margin:6px 0 4px 0;">GEX BY STRIKE  ·  {asset_toggle}</div>',
                unsafe_allow_html=True)
            _gex_row = plot_df.copy()
            _gex_row["strike_fut"] = _gex_row["strike"] * float(equiv_mult)
            _gex_max = float(_gex_row["gex_net"].abs().max()) or 1e-9
            _gc = [f"rgba(0,200,120,{0.4+0.6*min(abs(v)/_gex_max,1):.2f})"
                   if v >= 0
                   else f"rgba(220,40,60,{0.4+0.6*min(abs(v)/_gex_max,1):.2f})"
                   for v in _gex_row["gex_net"]]
            _fig_gs = go.Figure(go.Bar(
                x=_gex_row["gex_net"], y=_gex_row["strike_fut"],
                orientation="h",
                marker=dict(color=_gc, line=dict(width=0)),
                hovertemplate="Strike $%{y:,.0f}<br>GEX: %{x:.4f}B<extra></extra>",
            ))
            _fig_gs.add_hline(y=spot_price * float(equiv_mult),
                              line_color=T["t1"], line_width=1.5, line_dash="solid")
            _fig_gs.update_layout(
                template="none", paper_bgcolor=T["bg"], plot_bgcolor=T["bg"],
                height=320, margin=dict(t=8, r=8, b=36, l=60),
                xaxis=dict(title=dict(text="Net GEX ($B)", font=dict(size=8, color=T["t3"])),
                           gridcolor=T["line2"], zerolinecolor=T["line2"], zerolinewidth=1.5,
                           tickfont=dict(size=8, color=T["t3"])),
                yaxis=dict(gridcolor=T["line2"], tickprefix="$",
                           tickfont=dict(size=8, color=T["t3"]),
                           range=[spot_price*float(equiv_mult)*0.953,
                                  spot_price*float(equiv_mult)*1.047]),
                hoverlabel=dict(bgcolor=T["bg2"],
...
elif _active == "vix_regime":
    render_vix_regime_page()
else:
    dashboard()

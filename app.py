...
def render_greeks_3d_page():
    ...
    _SURFACES = [
        ("gamma",         "Γ Gamma Exposure (GEX)",
         [[0.00,"#FF0000"],[0.15,"#FF2200"],[0.30,"#FF4400"],[0.45,"#CC0000"],
          [0.499,"#880000"],[0.50,"#000000"],[0.501,"#008800"],
          [0.55,"#00AA00"],[0.70,"#00CC00"],[0.85,"#00EE00"],[1.00,"#00FF00"]]),
    ...
    for greek_key, greek_label, colorscale_3d in _SURFACES:
        ...
        # customdata = per-strike futures-converted prices
        _cd3_row = np.array([round(float(s) * float(_3d_ratio), 2) for s in strikes_3d])
        _cd3 = np.tile(_cd3_row, (len(exp_labels_3d), 1)).T
        ...
        fig_3d.update_layout(
            ...
            scene=dict(
                ...
                aspectmode="manual", aspectratio=dict(x=2.8, y=0.7, z=1.3),
                dragmode="turntable",
            ),
            showlegend=False,
            font=dict(family="JetBrains Mono, monospace", color=TEXT2_3, size=10),
        ...
        st.plotly_chart(fig_3d, use_container_width=True, config={
            ...
            "modeBarButtonsToRemove": ["sendDataToCloud","toImage","orbitRotation","hoverClosest3d"],
            "modeBarButtonsToAdd": ["tableRotation", "resetCameraDefault3d"],
        })
...
def dashboard():
    ...
    _modes_list  = ["GEX", "ES_CHART"]
    _labels_list = ["OI GEX", "🕯 ES Chart"]
    ...
    for _col, _mode, _lbl in zip(_btn_cols[:2], _modes_list, _labels_list):
        with _col:
            st.button(
                _lbl,
    ...
    if st.session_state.radar_mode not in ("GEX", "ES_CHART"):
    ...
    if st.session_state.radar_mode == "GEX":
        ...
        # ── Greek data mapping ────────────────────────────────────────────────
        _greek_col_map = {
            "GEX": ("gex_net",  T["green"],  T["red"],    "GEX $B — Dealer Gamma Exposure"),
            "DEX": ("dex_net",  T["blue"],   T["violet"], "DEX Δ$ — Delta Exposure"),
            "VEX": ("vex_net",  T["green"],  T["red"],    "VEX $M — Vanna Exposure (Sticky-Δ)"),
            "CEX": ("cex_net",  T["amber"],  T["violet"], "CEX — Charm Exposure (Sticky-Δ)"),
            "TEX": ("tex_net",  T["red"],    T["blue"],   "TEX $K/day — Theta Exposure"),
        }
        _gcol, _gcolor_pos, _gcolor_neg, _gunits = _greek_col_map.get(
            _selected_greek, ("gex_net", T["green"], T["red"], "GEX $B"))

        if _gcol in df.columns and not df.empty:
            _plot_df = df.copy()
            _plot_df["_val"] = _plot_df[_gcol].fillna(0)
            _plot_df["_fut_strike"] = _plot_df["strike"] * (equiv_mult or 1.0)
            _plot_df = _plot_df.sort_values("_fut_strike")
            _vals = _plot_df["_val"].tolist()
            _strikes = _plot_df["_fut_strike"].tolist()
            _bar_colors = [_gcolor_pos if v >= 0 else _gcolor_neg for v in _vals]

            if _display_mode == "BARS":
                # ── Horizontal bars ───────────────────────────────────────────
                _fig_gb = go.Figure()
                _fig_gb.add_trace(go.Bar(
                    x=_vals, y=[f"${s:,.0f}" for s in _strikes],
                    orientation="h",
                    marker=dict(
                        color=_bar_colors,
                        opacity=0.82,
                    ...
                    customdata=_strikes,
                    hovertemplate=(
                        f"<b>Strike: $%{{customdata:,.0f}}</b><br>"
                        f"{_selected_greek}: %{{x:+.4f}} {_gunits}<br>"
                        "<extra></extra>"
                    ),
                ))
                # Spot line
                _fig_gb.add_vline(
                    x=0, line_color=T["line2"], line_width=1, line_dash="solid"
                )
                _spot_nearest = min(_strikes, key=lambda s: abs(s - spot_price * (equiv_mult or 1.0)))
                _fig_gb.add_hline(
                    y=f"${_spot_nearest:,.0f}",
                    line_color=T["amber"], line_width=1.2, line_dash="dot",
                    annotation_text=f" SPOT {spot_price:.2f}",
                    annotation_font=dict(size=8, color=T["amber"]),
                )
                _gb_h = max(320, min(len(_strikes) * 22, 700))
                _fig_gb.update_layout(
                    template="none", paper_bgcolor=T["bg"], plot_bgcolor=T["bg"],
                    height=_gb_h,
                    margin=dict(t=24, r=18, b=32, l=80),
                    title=dict(
                        text=f"<b>{_selected_greek} EXPOSURE</b>  ·  {asset_toggle}  ·  {_display_mode}",
                        font=dict(size=10, color=T["t1"], family="JetBrains Mono"),
                        x=0.01, xanchor="left",
                    ),
                    xaxis=dict(
                        title=dict(text=_gunits, font=dict(size=8, color=T["t3"])),
                        gridcolor=T["line"], tickfont=dict(size=8), zeroline=True,
                        zerolinecolor=T["line2"],
                    ),
                ...
                # ── Heatmap (strike × expiry, if raw_df available) ────────────
                _hm_data = None
                if not raw_df.empty and "expiry" in raw_df.columns and _gcol in raw_df.columns:
                    _exps = sorted(raw_df["expiry"].unique())[:8]
                    _stk_range = raw_df[(raw_df["strike"] >= spot_price * 0.90) &
                                        (raw_df["strike"] <= spot_price * 1.10)]["strike"].unique()
                    _stk_range = sorted(_stk_range)
                    if len(_stk_range) > 1 and len(_exps) > 1:
                        _z_matrix = []
                        for _exp in _exps:
                            _row_df = raw_df[raw_df["expiry"] == _exp]
                            _row_vals = []
                            for _sk in _stk_range:
                                _row_sk = _row_df[_row_df["strike"] == _sk]
                                _v = float(_row_sk[_gcol].sum()) if not _row_sk.empty and _gcol in _row_sk.columns else 0.0
                                _row_vals.append(_v)
                            _z_matrix.append(_row_vals)
                        _hm_data = (_z_matrix, _stk_range, _exps)

                if _hm_data:
                    _z_m, _stk_l, _exp_l = _hm_data
                    _stk_fut = [s * (equiv_mult or 1.0) for s in _stk_l]
                    _fig_hm = go.Figure(go.Heatmap(
                        z=_z_m,
                        x=[f"${s:,.0f}" for s in _stk_fut],
                        y=[str(e) for e in _exp_l],
                        colorscale=T.get("heat_colorscale", "RdYlGn"),
                        zmid=0,
                        hovertemplate=(
                            f"Strike: $%{{x}}<br>Expiry: %{{y}}<br>"
                            f"{_selected_greek}: %{{z:+.4f}}<extra></extra>"
                        ),
                        colorbar=dict(
                            title=dict(text=_gunits,
                                       font=dict(size=8, color=T["t3"], family="JetBrains Mono")),
                            tickfont=dict(size=8, color=T["t3"], family="JetBrains Mono"),
                            bgcolor=T["bg"], outlinewidth=0, thickness=10,
                        ),
                    ))
                    # Spot marker
                    _spot_fut = spot_price * (equiv_mult or 1.0)
                    _fig_hm.add_vline(
                        x=f"${_spot_nearest:,.0f}" if _spot_nearest else 0,
                        line_color=T["amber"], line_width=1.5, line_dash="dot",
                        annotation_text=f" SPOT", annotation_font=dict(size=8, color=T["amber"]),
                    )
                    _fig_hm.update_layout(
                        template="none", paper_bgcolor=T["bg"], plot_bgcolor=T["bg"],
                        height=380,
                        margin=dict(t=28, r=10, b=60, l=90),
                        title=dict(
                            text=f"<b>{_selected_greek} HEATMAP</b>  ·  Strike × Expiry  ·  {asset_toggle}",
                            font=dict(size=10, color=T["t1"], family="JetBrains Mono"),
                            x=0.01, xanchor="left",
                        ),
                    ...
                    # Fallback: heatmap from aggregated df
                    _fig_hm2 = go.Figure(go.Bar(
                        x=[f"${s:,.0f}" for s in _strikes], y=_vals,
                        marker=dict(color=_bar_colors, opacity=0.82, line=dict(width=0)),
                        hovertemplate=f"Strike: $%{{x}}<br>{_selected_greek}: %{{y:+.4f}}<extra></extra>",
                    ))
                    _fig_hm2.update_layout(
                        template="none", paper_bgcolor=T["bg"], plot_bgcolor=T["bg"],
                        height=350, margin=dict(t=28, r=12, b=55, l=55),
                        xaxis=dict(tickangle=-35, tickfont=dict(size=8), gridcolor=T["line"]),
                        yaxis=dict(gridcolor=T["line"], tickfont=dict(size=8)),
                        title=dict(
                            text=f"<b>{_selected_greek} EXPOSURE</b>  ·  {asset_toggle}  ·  Heatmap (Aggregated)",
                            font=dict(size=10, color=T["t1"], family="JetBrains Mono"),
                            x=0.01,
                        ),
                        font=dict(family="JetBrains Mono,monospace", color=T["t2"], size=9),
        ...
        # ══════════════════════════════════════════════════════════════════════
        # INTRADAY SETUP GRADING SYSTEM (A+ to F)
        # ══════════════════════════════════════════════════════════════════════
        _setup_score = 0
        _setup_signals = []
        ...
        if is_long_gamma:
            _setup_score += 25
            _setup_signals.append(("✅ Long Gamma", "+25", T["green"]))
        else:
            _setup_score -= 15
            _setup_signals.append(("⚠️ Short Gamma", "-15", T["red"]))

        # GEX ratio signal
        if gex_ratio > 0.65:
            _setup_score += 20
            _setup_signals.append(("✅ Bullish GEX Ratio", "+20", T["green"]))
        elif gex_ratio < 0.35:
            _setup_score -= 10
            _setup_signals.append(("⚠️ Bearish GEX Ratio", "-10", T["red"]))
        else:
            _setup_score += 5
            _setup_signals.append(("🔹 Neutral GEX", "+5", T["amber"]))

        # IV-RV spread
        _iv_rv_val = iv_rv_spread if iv_rv_spread is not None else 0.0
        if _iv_rv_val > 2:
            _setup_score += 15
            _setup_signals.append(("✅ IV Premium (vol overpriced)", "+15", T["green"]))
        elif _iv_rv_val < -2:
            _setup_score -= 10
            _setup_signals.append(("⚠️ RV > IV (underpriced vol)", "-10", T["red"]))

        # Flow ratio
        if flow_ratio > 0.6:
            _setup_score += 20
            _setup_signals.append(("✅ Bullish Flow", "+20", T["green"]))
        elif flow_ratio < 0.4:
            _setup_score -= 15
            _setup_signals.append(("⚠️ Bearish Flow", "-15", T["red"]))

        # Spot vs call wall / put wall (proximity)
        if call_wall and put_wall and call_wall > put_wall:
            _cw_dist = abs(spot_price - call_wall) / spot_price * 100
            _pw_dist = abs(spot_price - put_wall)  / spot_price * 100
            if _cw_dist < 0.5:
                _setup_score -= 10
                _setup_signals.append(("⚠️ Near Call Wall resistance", "-10", T["red"]))
            elif _pw_dist < 0.5:
                _setup_score += 10
                _setup_signals.append(("✅ Near Put Wall support", "+10", T["green"]))

        # Normalize to 0-100
        _setup_score = max(0, min(100, _setup_score + 50))

        # Grade
        if   _setup_score >= 90: _grade, _grade_col = "A+", T["green"]
        elif _setup_score >= 80: _grade, _grade_col = "A",  T["green"]
        elif _setup_score >= 70: _grade, _grade_col = "B+", "#44FF99"
        elif _setup_score >= 60: _grade, _grade_col = "B",  T["amber"]
        elif _setup_score >= 50: _grade, _grade_col = "C+", T["amber"]
        elif _setup_score >= 40: _grade, _grade_col = "C",  "#FFAA44"
        elif _setup_score >= 30: _grade, _grade_col = "D",  T["red"]
        else:                    _grade, _grade_col = "F",  T["red"]

        _prob_profit = int(_setup_score * 0.7 + 15)  # heuristic 15–85%
        ...
        _sigs_html = "".join(
            ...
            f'<span style="font-family:JetBrains Mono,monospace;font-size:9px;'
            f'font-weight:600;color:{col};">{pts}</span></div>'
            for sig, pts, col in _setup_signals
        )

        st.markdown(f"""
        <div style="background:{T['bg1']};border:1px solid {T['line2']};
                    border-left:3px solid {_grade_col};border-radius:4px;
                    padding:14px 18px;margin:16px 0 8px 0;">
          <div style="display:flex;align-items:baseline;gap:16px;margin-bottom:10px;">
            ...
            <span style="font-family:'Barlow Condensed',sans-serif;font-size:38px;
                         font-weight:700;color:{_grade_col};line-height:1;">{_grade}</span>
            <div style="margin-left:auto;text-align:right;">
              <div style="font-size:8px;color:{T['t3']};letter-spacing:2px;margin-bottom:2px;">
                PROB. PROFIT</div>
              <div style="font-family:'Barlow Condensed',sans-serif;font-size:22px;
                          font-weight:700;color:{_grade_col};">{_prob_profit}%</div>
            </div>
          </div>
          <div style="height:4px;background:{T['bg2']};border-radius:2px;margin-bottom:10px;">
            <div style="height:100%;width:{_setup_score}%;background:{_grade_col};
